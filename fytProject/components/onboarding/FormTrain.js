...
export default class FormTrain extends React.Component {
  ...
  handleClick = event => {
    ...
    if (this.state.counter < this.state.nameholder.length - 1) {
      ...
      this.finishOnboarding(
        ...
        this.state.typeholder,
        this.state.inputholder
      );
    }
  };
  ...
  render() {
    let percent =
      ((this.state.counter + 1) / this.state.nameholder.length) * 100;
    percent = percent.toString() + "%";
    if (this.state.step == "SIGNUP") {
      return (
        <InputForm
          formFieldName={this.state.nameholder[this.state.counter]}
